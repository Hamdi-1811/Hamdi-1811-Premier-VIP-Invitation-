        ...
        <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-black/95 shadow-lg shadow-amber-500/10' : 'bg-transparent'} py-4 px-4 border-b border-amber-600/30`}>
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <nav className="hidden md:flex items-center space-x-8">
                    {navLinks.map((link) => (
                        <a key={link.text} href={link.href} className="text-white uppercase tracking-wider text-sm font-semibold hover:text-amber-400 transition-colors duration-300">
                            {link.text}
                        </a>
                    ))}
                </nav>
            ...
            {isMenuOpen && (
                <div className="md:hidden absolute top-full left-0 w-full bg-black/95 backdrop-blur-sm shadow-lg py-4">
                    <nav className="flex flex-col items-center space-y-4">
                       {navLinks.map((link) => (
                            <a key={link.text} href={link.href} className="text-white uppercase tracking-wider text-lg font-semibold hover:text-amber-400 transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>
                                {link.text}
                            </a>
                        ))}
                    </nav>
